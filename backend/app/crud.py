...
async def get_clients_within_distance(*,
    ...
    clients_within_distance: List[Client.id] = [
        client.id for client in session.exec(statement).all()
        if distance >= great_circle_distance(
            lat1=current_lat,
            lon1=current_lon,
...
async def get_list_filter_client(*,
    ...
    if filter_client.sex:
        statement = (statement
                     .where(Client.sex == filter_client.sex))
    if filter_client.name:
        statement = (statement
                     .where(Client.name
                            .ilike(f"%{filter_client.name}%")))
    if filter_client.surname:
        statement = (statement
                     .where(Client.surname
                            .ilike(f"%{filter_client.surname}%")))
...
async def create_client(*,
                        session: Session,
                        client_create: ClientCreate) -> Client:
    db_obj = Client.model_validate(
        client_create,
        update={
            "hashed_password": await get_password_hash(client_create.password)
        }
    )
    session.add(db_obj)
    session.commit()
...
async def update_client(*,
                        session: Session,
                        db_client: Client,
                        client_up: ClientUpdateLocation) -> Client:
...
async def check_existing_like(*,
                              session: Session,
                              liker_id: uuid.UUID,
                              liked_id: uuid.UUID) -> Optional[Like]:
    statement = (select(Like)
                 .where((Like.liker_id == liker_id)
...
async def find_like_client(*,
                           session: Session,
                           liked_id: uuid.UUID) -> Sequence[Optional[Like]]:
...
async def check_match_between(*,
                              liker_id: uuid.UUID,
                              list_liked: Sequence[Like]) -> Optional[Like]:
