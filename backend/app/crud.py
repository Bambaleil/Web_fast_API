...
from .models import Client, ClientCreate, ClientUpdateLocation, FilterClient, Like
from .utils import great_circle_distance


@cached(ttl=600)
async def get_clients_within_distance(
    *,
    session: Session,
    current_lat: float,
    current_lon: float,
    ...
    clients_within_distance: List[Client.id] = [
        client.id
        for client in session.exec(statement).all()
        if distance
...
async def get_list_filter_client(
    *, session: Session, filter_client: FilterClient, current_client
):
    ...
    if filter_client.distance:
        clients_within_distance_ids = await get_clients_within_distance(
            session=session,
...
async def create_client(*, session: Session, client_create: ClientCreate) -> Client:
    db_obj = Client.model_validate(
        client_create,
        update={"hashed_password": await get_password_hash(client_create.password)},
    )
    session.add(db_obj)
    session.commit()
...
async def update_client(
    *, session: Session, db_client: Client, client_up: ClientUpdateLocation
) -> Client:
...
async def check_existing_like(
    *, session: Session, liker_id: uuid.UUID, liked_id: uuid.UUID
) -> Optional[Like]:
...
async def create_like(
    *, session: Session, liker_id: uuid.UUID, liked_id: uuid.UUID
    ...
    db_obj = Like(liker_id=liker_id, liked_id=liked_id)
    session.add(db_obj)
    session.commit()
    session.refresh(db_obj)
    return db_obj
...
async def find_like_client(
    *, session: Session, liked_id: uuid.UUID
) -> Sequence[Optional[Like]]:
    statement = select(Like).where(Like.liker_id == liked_id, Like.match is False)
    db_obj: Sequence[Optional[Like]] = session.exec(statement).all()
    return db_obj


async def check_match_between(
    *, liker_id: uuid.UUID, list_liked: Sequence[Like]
) -> Optional[Like]:
...
async def update_match_user(
    *, session: Session, liked_obj: Like, liker_obj: Like
) -> None:
...
async def authenticate(
    *, session: Session, email: str, password: str
    ...
    db_client: Optional[Client] = await get_client_by_email(
