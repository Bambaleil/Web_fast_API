...
from ..deps import CurrentClient, SessionDep
...
async def create_client(
    *, session: SessionDep, client_in: Annotated[ClientCreate, Depends()]
) -> Optional[Client]:
    """
    Create new client
    ...
    if client_in.avatar:
        if client_in.avatar.size > 1024 * 1024 * 5:
            raise HTTPException(status_code=400, detail="The file size is over 5 MB")
...
async def filter_list_client(
    current_client: CurrentClient,
    session: SessionDep,
    filter_client: Annotated[FilterClient, Depends()],
) -> Sequence[Optional[Client]]:
    clients: Sequence[Optional[Client]] = await crud.get_list_filter_client(
        session=session, current_client=current_client, filter_client=filter_client
    )
    return clients
...
async def update_location_client(
    client_id: uuid.UUID,
    session: SessionDep,
    coordinate: Annotated[ClientUpdateLocation, Depends()],
) -> Client:
    db_client: Client = session.get(Client, client_id)
    if not db_client:
        raise HTTPException(
    ...
    client: Client = await crud.update_client(
        session=session, db_client=db_client, client_up=coordinate
    )
    return client
