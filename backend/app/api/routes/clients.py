...
from app.models import ClientCreate, ClientPublic, Client, FilterClient, ClientUpdateLocation
from ... import crud
...
async def create_client(*, session: SessionDep, client_in: Annotated[ClientCreate, Depends()]) -> Optional[Client]:
    """
    Create new client
    ...
    if client_in.avatar:
        ...
        avatar_with_watermark = await add_watermark_with_photo(avatar=avatar_bytes)
...
async def filter_list_client(current_client: CurrentClient, session: SessionDep,
                             filter_client: Annotated[FilterClient, Depends()]) -> Sequence[Optional[Client]]:
    clients: Sequence[Optional[Client]] = await crud.get_list_filter_client(session=session,
...
async def update_location_client(client_id: uuid.UUID, session: SessionDep,
    ...
    client: Client = await crud.update_client(session=session, db_client=db_client, client_up=coordinate)
    return client
